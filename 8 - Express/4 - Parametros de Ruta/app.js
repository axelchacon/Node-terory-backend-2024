...
app.get("/api/cursos/programacion/:lenguajesos", (req, res) => {
  const milenguaje = req.params.lenguajesos;
  const resultados = infoCursos.programacion.filter(
    (curso) => curso.lenguaje === milenguaje
  );

  if (resultados.length === 0) {
    return res.status(404).send(`No se encontraron cursos de ${milenguaje}`);
  }
